...
* 网络问题：部分地区网络访问FL服务器延迟较高，请等待数据加载，或确保网络畅通重启软件。
...
* 华为手机：华为近期(2019年)发布的EMUI内核不支持ptrace操作，目前只有刷第三方解除了限制的内核才能使用ROOT模式，建议可以去xda论坛找找第三方内核
...
* NOROOT模式只能模拟GPS定位方式，模拟混合定位方式(GPS、WIFI、基站)则需要使用ROOT模式（手机没有ROOT的话可以参考下面第 9 条）。
  
* 模拟功能不会对打卡、配送、校园跑、网约车等可能违法违规的使用场景开放，使用条款里和开通专业版须知里都有相关注明，请悉知！
...
这种情况通常是程序被系统或其它某些应用清理掉了，请尝试允许并锁定FL后台运行，以及关闭省电优化之类的设置：
...
手机没有ROOT权限的话，可以尝试在光速虚拟机、VMOSPro等虚拟机里使用ROOT模式，安装虚拟机后，把 Fake Location 和需要使用Fake Location功能的应用一并安装进虚拟机内使用，就可以使用ROOT模式下的功能了。(提示：不保证所有虚拟机都能正常运行，还请自行测试)。
